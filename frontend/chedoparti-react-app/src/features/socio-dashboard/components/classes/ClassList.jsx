 ...
 */
export default function ClassList({ groupedClasses, onConfirm, onDecline, onDetailClick }) {
  const expandedSections = useSocioExpandedSections();
  const { toggleSection } = useSocioActions();

  if (!groupedClasses || groupedClasses.length === 0) {
    return null;
  }

  return (
    <div className="space-y-4">
      {groupedClasses.map(([dateKey, data]) => {
